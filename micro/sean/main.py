"""This module controls Ubisoft Stock (UBSFY)"""
...
def query_db(sql):
    ...
    res = db.create_engine(os.getenv('SEAN_DB')).connect().execute(sql)
...
def quotes():
    """returns a tradier quote for Ubisoft stock"""
    ...
    quote = json.loads('{}')
    conn.request('GET', '/v1/markets/quotes?symbols=UBSFY', None, headers)
    try:
